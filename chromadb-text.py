...
query = 'can ozone kill salmonella?'
...
related_questions = gen_aux_questions()
...
# TODO: find a way to choose the document (abstract) that better supports your case
#       that also have the most complete data
best_results = []
best_results.append(unique_results[ordered_results[0]])
best_results.append(unique_results[ordered_results[1]])
best_results.append(unique_results[ordered_results[2]])
best_results.append(unique_results[ordered_results[3]])
best_results.append(unique_results[ordered_results[4]])
done = False
for best_result in best_results:
    ...
    prompt = f'''
        Write 1 paragraph about herbal teas for bad breath using the CONTEXT below.
        The paragraph is 3 sentence.
        The sentences are short, but with a high density of facts.
        The paragraphs follow the typical structure of an abstract from a scientific paper: introduction, methods, results.
        In sentence 1, write the introduction, starting with the following words: "According to a study published by {journal_title}, ".
        In sentence 2, write the methods, including 1-2 data points, statistics, or numbers.
        In sentence 3, write the results, focusing on the fact that herbs are good for bad breath.
        Include only positive info, don't include negative info.
        Reply with "NOT ENOUGH CONTEXT" if the context provided doesn't give enough info to write the paragraphs.
        Write the paragaraph using only information from the context, don't make up an answer.
        ## CONTEXT
        {context}
    '''
    reply = util_ai.gen_reply(prompt, model)
...
prompt = f'''
    reply to the following QUESTION using the following CONTEXT.
    QUESTION: {query}
    CONTEXT: {context}
'''
reply = util_ai.gen_reply(prompt, model)
